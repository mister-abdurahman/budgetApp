    ...
    return (
        <div className='space-y-6'>
            ...
            <div className="grid grid-cols-12 gap-4">
                {menus.map((x, index) => {
                    return (
                        <Link to={x.url} key={index} className="flex col-span-4 gap-4 p-2 bg-white rounded-md shadow-lg">
                            ...
                            <div className="self-end font-bold">{filter_user_by_role(x.role).length}</div>
                        </Link>
                    )
                })}
            </div>
            <Modal page={<UserEdit handleModal={handleModal} title={"Profile"} />} isOpen={isOpen} />
        </div>
    )
}
